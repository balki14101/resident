...
import { View, Text, StyleSheet, ImageBackground, TouchableOpacity, Image, Button, BackHandler, Alert } from 'react-native'
...
export default function Home(props) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setLoading(true)
                const { data } = await axios.get('https://residentnow-backend.trikaradev.xyz/api/get-social-feed-posts', { headers: { Authorization: `Bearer ${userData.token}` } })
                console.log('social' ,data.socialFeedPosts[0])
                setSocial(data.socialFeedPosts)
                const { data: sticky } = await axios.get('https://residentnow-backend.trikaradev.xyz/api/get-sticky-post', { headers: { Authorization: `Bearer ${userData.token}` } })
                // console.log(sticky)
    ...
    return (
        <View style={{ height: '100%' }}>
            <Image source={BGImage} style={{ width: '100%', height: '35%', alignItems: 'center', borderBottomLeftRadius: 10, borderBottomRightRadius: 10 }}>
            </Image>
            <ScrollView style={{ position: 'absolute', width: '100%', height: '100%' }}>
                ...
                <View style={{ paddingStart: 13, paddingEnd: 13, marginBottom: 50 }}>
                    ...
                    <View style={styles.welcomeContainer}>
                        {userData &&
                            <Text style={styles.welcomeTxt}>Hello Mr {userData.user.first_name + ' ' + userData.user.last_name} ,</Text>
                        }
                        <Collapse>
                            <CollapseHeader>
                    ...
                    {isLoading &&
                        <ShimmerPlaceHolder
                            style={{ width: '100%', height: 100, marginTop: 50, borderRadius: 8 }}
                    ...
                    {!isLoading &&
                        ...
                        <View style={styles.communityContainer}>
                            <Text style={styles.communityHead}>{sticky.provider.business_name} Community</Text>
                    ...
                    {isLoading &&
                        <ShimmerPlaceHolder
                            style={{ width: '100%', height: 120, marginTop: 50, borderRadius: 8 }}
                            LinearGradient={LinearGradient}
                        />
                    }
                    {!isLoading &&
                        ...
                        social.map((item) =>
                            <View style={styles.welcomeContainer}>
                                <View style={{ flexDirection: "row" }}>
                                    <Image source={{uri: item.resident.user.profile_picture_url}} style={styles.profilePic}></Image>
                                    <View style={{ marginTop: 3 }}>
                                        <Text style={styles.communityHead}>{item.resident.user.first_name + ' ' + item.resident.user.last_name}</Text>
                                        <Text style={styles.communityTime}>Today 11.30 AM</Text>
                                    </View>
                                </View>
                                <View style={styles.line}></View>
                                <Text style={[styles.communityHead, { fontSize: 16, marginTop: 10 }]}>{item.post_text}</Text>
                                <View style={{ flexDirection: 'row' }}>
                                    {item.images.map((img) =>
                                        <Image source={{ uri: img.image_url }} style={[styles.gallery, { marginRight: 10 }]}></Image>
                                    )}
                                ...
                                {item.comments.map((comment) =>
                                    <View>
                                        <View style={{ flexDirection: "row" }}>
                                            <Image source={{uri:comment.commenting_user.profile_picture_url}} style={styles.profilePicSmall}></Image>
                                            <View style={{ marginTop: 3 }}>
                                                <Text style={[styles.communityHead, { fontSize: 14 }]}>{comment.commenting_user.first_name + ' ' + comment.commenting_user.last_name}</Text>
                                            </View>
                                        </View>
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingTop: 10 }}>
                                    <Text style={[styles.communityHead, { color: '#B96D05', fontSize: 14, marginTop: 6 }]}>{item.comments.length} Comments</Text>
                                    <View style={{ flexDirection: 'row' }}>
                                        <Text style={[styles.communityHead, { color: '#616D2F', marginRight: 8, marginTop: 6 }]}>{item.likes.length}</Text>
